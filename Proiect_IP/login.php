...
if (isset($_POST["login"])) {
	...
	while ($row = mysqli_fetch_row($query)) {
		...
		elseif ($row[1] == $username && ($row[2] == $password) && ($row[3] == "medic")) {
			header("location:medic/homeadmin.php?id_medic=" . $row[0]);
			$eroare = 0;
		}
		elseif ($row[1] == $username && ($row[2] == $password) && ($row[3] == "supraveghetor")) {
			header("location:supraveghetor/homeadmin.php?id_supraveghetor=" . $row[0]);
			$eroare = 0;
		}
		elseif ($row[1] == $username && ($row[2] == $password) && ($row[3] == "ingrijitor")) {
			header("location:ingrijitor/homeadmin.php?id_ingrijitor=" . $row[0]);
			$eroare = 0;
		}
		elseif ($row[1] == $username && ($row[2] == $password) && ($row[3] == "pacient")) {
			header("location:pacient/homeadmin.php?id_click=" . $row[0]);
			$eroare = 0;
		}
	}
	if($eroare==1)
		header("location:login.php?eroare=" . $eroare);
}
?>
<!DOCTYPE html>
<html>
				...
				<form class="login100-form validate-form" method="post">
					...
					if (isset($_GET["eroare"]))
						...
						<div class="alert alert-danger" role="alert">
							<h3><center>Combinatia de nume si parola este gresita!</center></h3>
						</div>
					<?php endif; ?>
