	...
	<div class="limiter">
		<table class="table table-striped table-responsive-sm">
			...
			<tbody>
				...
				$sql = "SELECT nume, prenume, cnp, data_nasterii, sex, adresa, telefon FROM utilizator";
				$sql_id = "SELECT id FROM utilizator";
				$sql_tip= "SELECT tip FROM utilizator";
				$result = $con->query($sql);
				$result_id = $con->query($sql_id);
				$result_tip = $con->query($sql_tip);
				while ($row = $result->fetch_assoc()) {
					$tip = $result_tip->fetch_assoc();
					$tip = $tip["tip"];
					if($tip=="pacient")
					{
						$id_click = $result_id->fetch_assoc();
						$id_click = $id_click["id"];
